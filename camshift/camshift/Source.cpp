...
public:
	...
	MatND getHueHistogram(const Mat &image,int minSaturation=0)
	{
		...
		if (minSaturation>0)
		{
			...
			threshold(v[1],mask,minSaturation,210,THRESH_BINARY);
		}
		// Compute histogram
...
int main()
{
	...
	Mat frame;
	capture.read(frame);
	cv::Rect rect(640,365,178,73);
	Mat imageROI= frame(rect);
	Mat hsv;
	ColorHistogram hc;
	ContentFinder finder;
	int minSat=80;int channels[]={0};
	namedWindow("image 2");
	while(1)
	{
		...
		cv::TermCriteria criteria(cv::TermCriteria::MAX_ITER,5,0.1);
